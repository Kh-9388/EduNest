...
export function TeacherCoursesPage() {
  ...
  const statusBadge = (s: string) => {
    switch (s) { case 'active': return <Badge className="bg-green-100 text-green-700">{s}</Badge>; case 'upcoming': return <Badge className="bg-blue-100 text-blue-700">{s}</Badge>; case 'completed': return <Badge className="bg-gray-100 text-gray-700">{s}</Badge>; default: return <Badge>{s}</Badge>; }
  };

  return (
    <div className="space-y-6">
      <div><h2 className="text-2xl font-bold text-gray-900">My Courses</h2><p className="text-sm text-gray-500 mt-1">Courses you are teaching</p></div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        courses.length === 0 ? <div className="col-span-full text-center text-gray-500 py-8">No courses assigned</div> :
        courses.map(c => (
          <Card key={c.id} className="border-0 shadow-sm hover:shadow-md transition-shadow">
            <CardContent className="p-5">
              ...
              <div className="flex items-center justify-between text-xs text-gray-500">
                <div className="flex items-center gap-1"><Users className="h-3 w-3" />{c.enrolled_count}/{c.capacity}</div>
                <div className="flex items-center gap-1"><Calendar className="h-3 w-3" />{c.start_date} - {c.end_date}</div>
              </div>
            </CardContent>
          </Card>
        ))}
